...
const std::unordered_map <std::string, SceneLoader::ParseEntityMethod> SceneLoader::parseMethodByEntityName =
{
	...
	{"BoomerangBro", &SceneLoader::ParseBoomerangBrother},
...
LPEntity SceneLoader::ParseBoomerangBrother(const std::vector<std::string>& tokens)
{
	if (tokens.size() != 4)
		throw InvalidTokenSizeException(4);

	return new Entities::BoomerangBrother({ stof(tokens[1]), stof(tokens[2]) });
}
...
LPEntity SceneLoader::ParseItemWoodBlock(const std::vector<std::string>& tokens)
{
	if (tokens.size() != 5)
		throw InvalidTokenSizeException(5);

	return new Entities::ItemWoodBlock(tokens[1], Vector2<float>(stof(tokens[2]), stof(tokens[3])));
}
